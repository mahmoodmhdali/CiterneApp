...
@Component({
  ...
  templateUrl: './ngx-event-images-popup.component.html'
})
export class NgxEventImagesPopupComponent implements OnInit, AfterViewInit {
  ...
  constructor (
    ...
    public dialogRef: MatDialogRef<NgxEventImagesPopupComponent>,
    ...
    this.eventsService.editImages(formData).subscribe(
      (responseBuilder: ResponseBuilderModel) => {
        ...
        if (responseBuilder.code === + this.apiConfig.SUCCESS) {
          ...
          this.snack.open('Event Images Updated Successfully', 'OK', {duration: 4000});
