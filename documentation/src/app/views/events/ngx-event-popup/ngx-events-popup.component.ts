...
export class NgxEventsPopupComponent implements OnInit, AfterViewInit {
  ...
  buildItemForm (item) {
    ...
    if (item.eventClassCastAndCredits) {
      ...
      for (const castAndCredit of item.eventClassCastAndCredits) {
        this.addEventClassCastAndCredits(castAndCredit.title, castAndCredit.description, false);
      }
    }
  ...
  addEventClassCastAndCredits (titleValue, descriptionValue, changeState) {
    if (changeState) {
